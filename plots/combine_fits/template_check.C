...
void template_check(){
    ...
    h_elel_back->SetFillColor(kGreen +3);
    h_elel_dy->SetFillColor(kRed+1);

    h_mumu_back->SetFillColor(kGreen +3);
    h_mumu_dy->SetFillColor(kRed+1);

    TCanvas *c_elel = new TCanvas("c_elel", "Histograms", 200, 10, 900, 700);
    h_elel_dy->Draw("hist");
    h_elel_back->Draw("hist same");

    TCanvas *c_mumu = new TCanvas("c_mumu", "Histograms", 200, 10, 900, 700);
    h_mumu_dy->Draw("hist");
    h_mumu_back->Draw("hist same");

    c_elel->Print("elel_template_check.pdf");
    c_elel->Print("mumu_template_check.pdf");
