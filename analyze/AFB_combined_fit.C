...
void AFB_combined_fit(){
    ...
    Double_t AFB_fit = minuit->GetParameter(0); 
    Double_t AFB_fit_error = minuit->GetParError(0);
    Double_t r_elel_back_fit = minuit->GetParameter(1); 
    Double_t r_elel_back_fit_error = minuit->GetParError(1);
    Double_t r_mumu_back_fit = minuit->GetParameter(2); 
    Double_t r_mumu_back_fit_error = minuit->GetParError(2);



    printf("\n\n\n Fit on M=[%.0f, %.0f], %i ElEl Events, %i MuMu Events: AFB = %0.3f +/- %0.3f r_elel_back = %0.3f +/- %0.3f r_mumu_back =%0.3f +/- %0.3f \n", 
                m_low, m_high, nElEl_DataEvents, nMuMu_DataEvents, AFB_fit, AFB_fit_error, r_elel_back_fit, r_elel_back_fit_error, r_mumu_back_fit, r_mumu_back_fit_error);
