...
const TString fout_name("AFB_fit/fit_results/m_bins/combined_fit_test_may25.root");
...
TH2F *h_elel_asym, *h_elel_sym, *h_elel_back,  *h_elel_data;
...
void gen_fit_templates_from_finite_mc(TH2F *h_sym, TH2F *h_asym, TH2F *h_mc, Double_t alpha){
    for(int i=1; i<=n_xf_bins; i++){    
        for(int j=1; j<=n_cost_bins; j++){
            Double_t cost_center = (cost_bins[j-1] + cost_bins[j])/2.;
            Double_t xf_center = (xf_bins[i-1] + xf_bins[i])/2.;
            ...
            printf("cost, xf ,rw : %.2f, %.2f %.2f \n", cost_center, xf_center,reweight);
            h_sym->Fill(xf_center, cost_center, bin_weight);
            h_sym->Fill(xf_center, -cost_center, bin_weight);

            h_asym->Fill(xf_center, cost_center, reweight *bin_weight);
            h_asym->Fill(xf_center, -cost_center, -reweight *bin_weight);
        }
    }
...
void setup(){
    ...
    //gen_finite_mc_template(t_elel_mc, h_elel_mc, h_elel_mc_errs, h_elel_sym_count, m_low, m_high, FLAG_ELECTRONS);
    //gen_fit_templates_from_finite_mc(h_elel_sym, h_elel_asym, h_elel_mc, alpha);
    print_hist(h_elel_sym);
    print_hist(h_elel_asym);
