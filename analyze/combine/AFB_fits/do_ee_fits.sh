...
do
    ...
    PostFitShapesFromWorkspace -w higgsCombineTest.MultiDimFit.mH120.root -f multidimfit.root:fit_mdf --postfit -o shapes/ee_fit_shapes_mbin${idx}.root
    echo "fit_mdf->Print();" > cmd.txt
    echo ".q" >> cmd.txt
    root -l -b multidimfit.root < cmd.txt > fit_results/ee_fit_results_mbin${idx}.txt
